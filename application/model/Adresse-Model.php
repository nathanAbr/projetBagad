<?php
class Adresse_Model Extends core.Model {


    function loadAdresse() {
        $tableau = array();
        $i = 0;
        $reponse = $bdd->query('SELECT idAdresse, rue1, rue2, complement, l.ville, l.pays, l.codePostale
						        FROM adresse 
    ...
    function insertAdresse($Adresse) {
        ...
        $req->prepare('INSERT INTO adresse(rue1, rue2, complement, fk_localite)
    ...
    function updateAdresse($Adresse) {
        ...
        $req = $bdd->prepare('UPDATE adresse SET rue1="'.$Adresse['rue1'].'", rue2="'.$Adresse['rue2'].'", complement="'.$Adresse['complement'].'", fk_localite="'.$Adresse['idLocalite'].'"
    ...
    function deleteAdresse($Adresse) {
        $req = $bdd->prepare('DELETE FROM adresse WHERE idAdresse ="'.$Adresse['id'].'"');
        $req->execute();
        if($req){
