<?php 
class Membre_Model Extends core.Model { 
    
    
    function loadMembre() { 
	    $tableau = array();
	    $i = 0;
        $reponse = $bdd->query('SELECT m.idMembre, m.mail, m.nom, m.prenom, m.dateNaissance, m.telephone, m.login, m.motDePasse,
    ...
    $req->prepare('INSERT INTO membre(mail, nom, prenom, dateNaissance, telephone, login, motDePasse, fk_instrument, fk_adresse, fk_groupe)
	...
	function updateMembre($Membre) { 
		...
		$req = $bdd->prepare('UPDATE membre SET mail="'.$Membre['mail'].'", nom="'.$Membre['nom'].'", prenom="'.$Membre['prenom'].'", dateNaissance="'.$Membre['dateNaissance'].'"
	...
	function deleteMembre() { 
		$req = $bdd->prepare('DELETE FROM membre WHERE nom ="'.$Membre['nom'].'" AND prenom = "'.$Membre['prenom'].'"');
		$req->execute();
		if($req){
